...
Animator.prototype.updateProps_ = function(timestamp) {
  ...
  for (var i = 0; i < this.start_.length; i++) {
    var progress = (timestamp - this.startTime_) / this.duration_;
    this.setValueFor_(i, this.start_[i] + progress * (this.end_[i] - this.start_[i]));
  }
...
Animator.prototype.finalize_ = function() {
  for (var i = 0; i < this.start_.length; i++) {
    this.setValueFor_(i, this.end_[i]);
  }
  this.resolved_ = true;
  this.callback_();
};

Animator.prototype.setValueFor_ = function(i, value) {
  this.setValue_(this.props_[i], this.fns_[i](value));
};
...
FADE_DOWN_ANIMATION = function(el) {
  return new Animator(el, ['margin-top', 'opacity'], [5, 1.0], [25, 0.0],
      [withpx, identity], 150);
};

FADE_UP_ANIMATION = function(el) {
  el.style.opacity = '0';
  return new Animator(el, ['margin-top', 'opacity'], [25, 0.0], [5, 1.0],
      [withpx, identity], 150);
};
